...
char * clientComm(int clntSockfd,int * senderBuffSize_addr, int * optlen_addr){
    ...
    /* For finding the vulnerability, I carefully combed
    through the clientComm function and I noticed that the string copy function
    receiving from the buffer was just copying everything without
    a limit on the data ize. Therefore, I changed it to strncpy
    instead and added the MAX_DATA_SIZE parameter. */
